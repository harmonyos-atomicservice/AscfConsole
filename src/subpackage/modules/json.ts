...
import { getMpViewType, has, log } from '@/main/modules/util';
import type {
    JSONValue,
    JSONChunk,
    JSONProp,
...
export const getJSONType = (obj: any): JSONType | undefined => {
...
export const getGlobalObjectConstructor = (
    clsName: GlobalObjectConstructorNames
): AnyFunction | AnyClass | undefined => {
    if (clsName === 'Number') {
        return Number;
    }
    if (clsName === 'Array') {
    ...
    const setPolyfill = () => {
        polyfill = function getOwnPropertyDescriptors(object, filter?: JSONPropFilter) {
            ...
            const res = Reflect.ownKeys(object).reduce((res, key, index) => {
                maxIndex = index;
                const val = Object.getOwnPropertyDescriptor(object, key) as PropertyDescriptor;
                let pass = true;
                if (typeof filter === 'function') {
        ...
        mock = (object, filter?: JSONPropFilter) => {
            ...
            for (const prop in object) {
                if (has(object, prop)) {
                    index++;
                    let pass = true;
                    const val = Object.getOwnPropertyDescriptor(object, prop) as PropertyDescriptor;
                    if (typeof filter === 'function') {
...
export const getJSONTree = (
    ...
    const res: JSONTree = [];
    let filter: JSONPropFilter | undefined;
    // TODO:属性过多时，需要处理
    if (startPropIndex && endPropIndex && startPropIndex >= 0 && endPropIndex > 0) {
        let index = -1;
        filter = (): boolean => {
            index++;
